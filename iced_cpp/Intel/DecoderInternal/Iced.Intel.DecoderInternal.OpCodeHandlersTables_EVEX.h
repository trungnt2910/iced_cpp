// C# helper headers
#include <csharp/classes.h>
#include <csharp/enum.h>
#include <csharp/interfaces.h>
#include <csharp/primitives.h>

// Commonly used headers
#include <cstdint>
#include <format>
#include <functional>
#include <memory>
#include <stdexcept>
#include <string>
#include <vector>

#pragma once

#include "OpCodeHandlers.h"
#include <vector>

// Code generated from Iced. Do not edit.
// Commit tag: badb6147c0994a4954fa27645aba2b02c2bb9502.
// SPDX-License-Identifier: MIT
// Copyright (C) 2018-present iced project and contributors
// ⚠️This file was generated by GENERATOR!🦹‍♂️
namespace Iced::Intel::DecoderInternal
{
	class OpCodeHandlersTables_EVEX final
	{
	public:
		static std::vector<std::shared_ptr<OpCodeHandler>> Handlers_0F;
		static std::vector<std::shared_ptr<OpCodeHandler>> Handlers_0F38;
		static std::vector<std::shared_ptr<OpCodeHandler>> Handlers_0F3A;
		static std::vector<std::shared_ptr<OpCodeHandler>> Handlers_MAP5;
		static std::vector<std::shared_ptr<OpCodeHandler>> Handlers_MAP6;
	private:
		class StaticConstructor
		{
		public:
			StaticConstructor();
		};

	private:
		static OpCodeHandlersTables_EVEX::StaticConstructor staticConstructor;



		static std::vector<std::uint8_t> GetSerializedTables();
		static constexpr std::int32_t MaxIdNames = 10;
		static constexpr std::uint32_t Handlers_0FIndex = 9;
		static constexpr std::uint32_t Handlers_0F38Index = 5;
		static constexpr std::uint32_t Handlers_0F3AIndex = 6;
		static constexpr std::uint32_t Handlers_MAP5Index = 7;
		static constexpr std::uint32_t Handlers_MAP6Index = 8;
	};
}
