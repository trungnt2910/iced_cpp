...
{
	using TryConvertToDisp8N = std::function<bool(Encoder* encoder, std::shared_ptr<OpCodeHandler> handler, Instruction const instruction, std::int32_t displ, std::int8_t& compressedValue)>;
	class OpCodeHandler
	{
	public:
	...
	private:
		...
		{
		public:
			static bool TryConvertToDisp8N(Encoder* encoder, std::shared_ptr<OpCodeHandler> handler, Instruction const instruction, std::int32_t displ, std::int8_t& compressedValue);
		};
	public:
		void Encode(Encoder* encoder, Instruction const instruction) override;
	};
